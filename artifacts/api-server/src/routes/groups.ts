...
router.patch("/:id/accept", requireAuth, async (req, res) => {
  ...
  if (!group.memberIds.includes(req.user!.id)) { res.status(403).json({ error: "Not a member of this group" }); return; }

  trackIfConsented(req, "match_accepted", req.user!.id, { groupId, city: group.city });
  res.json({ ok: true });
});
...
router.patch("/:id/decline", requireAuth, async (req, res) => {
  ...
  trackIfConsented(req, "match_declined", req.user!.id, { groupId, city: group.city });
  res.json({ ok: true });
});
